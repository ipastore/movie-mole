...
3. **Normalize before reading the diff.** Run `pnpm quality` (or at minimum
   `biome check --write .`). Codex runs `--yolo` and writes files directly, so a pre-commit
   hook cannot intercept mid-build — without this you review formatting noise instead of
   logic. Auto-fixable findings are fixed here, not fed back to Codex.
